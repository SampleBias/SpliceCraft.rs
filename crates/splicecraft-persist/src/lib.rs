...
mod tests {
    ...
    #[test]
    fn domain_gels_save_is_sandboxed() {
        let (tmp, layout) = sandbox();
        assert!(layout.root.starts_with(tmp.path()));
        save_gels(
            &layout,
            &[json!({"id":"gel-aaaaaaaa","name":"Friday digest"})],
        )
        .unwrap();
        let loaded = load_gels(&layout);
        assert_eq!(loaded.entries[0]["id"], "gel-aaaaaaaa");
        assert!(layout.gels_file().starts_with(&layout.root));
        assert_eq!(
